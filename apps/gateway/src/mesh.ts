...
export class MeshRelay {
  ...
  async send(from: string, to: string, kind: MeshMessageKind, body: string): Promise<MeshMessage> {
  ...
  private async inject(msg: MeshMessage): Promise<void> {
    const label = msg.from === 'user' ? '用户代发' : msg.from === 'scheduler' ? '定时任务' : `来自 ${msg.from}`;
  ...
  private handleStatusChange(agentId: string, from: string, to: string): void {
    this.pushTimeline({ kind: 'status', from: agentId, text: `${from} → ${to}`, pendingAction: to === 'blocked' ? 'blocked' : null });
    ...
    for (const rule of this.rules) {
      if (!rule.enabled || rule.watcherAgent !== agentId || rule.triggerState !== to) continue;
  ...
  private recentPairCount(from: string, to: string): number {
    ...
    return this.messages.filter((m) => m.from === from && m.to === to && m.createdAt > cutoff && m.status !== 'denied').length;
  }
  ...
  private loopDepth(from: string, to: string): number {
    ...
    const recent = this.messages
      .filter((m) => m.createdAt > cutoff && ((m.from === from && m.to === to) || (m.from === to && m.to === from)))
