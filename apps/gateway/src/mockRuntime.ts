...
export class MockRuntime implements AgentRuntime {
    ...
    this.addAgent(
      {
        ...
        turn: 3,
        unread: 1,
        config: defaultConfig('gpt-5.3-codex', { permissionMode: 'ask', reasoning: 'medium' }),
      },
      [
    ...
    this.addAgent(
      {
        ...
        turn: 1,
        unread: 0,
        config: defaultConfig('gemini-3-pro', { permissionMode: 'plan', reasoning: 'medium', contextWindow: '200k' }),
      },
      ['❯ (空闲) 等待指令…'],
    );
    this.addAgent(
    ...
    this.addAgent(
      {
        ...
        turn: 0,
        unread: 0,
        config: defaultConfig('claude-haiku-4-5', { reasoning: 'low', contextWindow: '200k', permissionMode: 'ask' }),
      },
      ['❯ (空闲) 等待指令…'],
    );
  }
  ...
  listWorkspaces(): WorkspaceSnapshot[] {
    const count = (ws: string) => [...this.agents.values()].filter((a) => a.snap.workspaceId === ws).length;
    return [
      { id: 'ws-acme', label: 'acme-api', agentCount: count('ws-acme'), focused: true },
      { id: 'ws-mkt', label: 'marketing-site', agentCount: count('ws-mkt'), focused: false },
    ];
  }
