...
import type { AgentSnapshot, WorkspaceSnapshot, AgentConfig, AgentStatus } from '@workbench/contracts';
...
export class HerdrRuntime implements AgentRuntime {
  ...
  private handleEvent(data: Record<string, unknown> & { type?: string }): void {
    ...
    if (type === 'pane.agent_status_changed' || type === undefined) {
      // 状态事件携带 pane_id + agent_status；其余事件全量刷新
    }
    if (type && ['pane_created', 'pane_closed', 'pane_agent_detected', 'workspace_created', 'workspace_closed'].some((t) => type.includes(t.replace('pane_', 'pane').replace('workspace_', 'workspace')))) {
    ...
    if (paneId && status) {
      ...
      if (tracked && tracked.snap.status !== status) {
        ...
        if (status === 'working') this.turns.set(tracked.snap.id, (this.turns.get(tracked.snap.id) ?? 0) + 1);
  ...
  private async refresh(): Promise<void> {
    const [agentsRes, wsRes] = await Promise.all([this.rpc.agentList(), this.rpc.workspaceList()]);
    ...
    for (const info of list) {
      ...
      next.set(id, {
        ...
        snap: {
          ...
          unread: prev?.snap.unread ?? 0,
          config,
        },
      });
    }
    this.agents = next;
    const wsList = (wsRes.workspaces ?? []) as { workspace_id: string; label?: string | null; focused?: boolean }[];
