...
export class ConfigService {
  ...
  async apply(agentId: string, next: AgentConfig): Promise<AgentApplyConfigResult> {
    ...
    if (next.model !== cur.model) {
      await handle(`模型 → ${modelLabel(next.model)}`, cap.sessionModelSwitch, `/model ${next.model}`, {
        model: next.model,
      });
    }
    if (next.reasoning !== cur.reasoning) {
      await handle(`推理强度 → ${next.reasoning}`, cap.reasoningSwitch, null, { reasoning: next.reasoning });
    }
    if (next.contextWindow !== cur.contextWindow) {
      await handle(`上下文 → ${next.contextWindow}`, cap.reasoningSwitch, null, { contextWindow: next.contextWindow });
    }
