...
import type { ConfigPreset, GatewaySettings } from "@workbench/contracts";
...
async function pickRuntime(): Promise<AgentRuntime> {
    ...
    if (forced !== "mock" && socketAvailable()) {
        try {
            ...
            console.warn(
                "[gateway] herdr 连接失败，降级 mock 模式:",
                err instanceof Error ? err.message : err,
            );
        }
    } else if (forced !== "mock") {
        console.log(
            "[gateway] 未发现 herdr socket，使用 mock 模式（启动 herdr 后重启 gateway 即接管真实 pane）",
        );
    }
    ...
    mesh.timeline.push(
        {
            ...
            from: "claude-e2e",
            to: "claude-backend",
            text: "注册 wait-for-state: done → 自动运行 e2e 套件",
            pendingAction: null,
        },
        {
            id: "seed-t1",
            ...
            to: "codex-review",
            messageId: "seed-1",
            text: "src/auth 重构完成，请 review 本轮 diff…",
            pendingAction: null,
        },
        {
            id: "seed-t2",
        ...
        },
        {
            ...
            kind: "status",
            from: "codex-review",
            text: "working → blocked（shell 权限确认：npm audit fix）",
