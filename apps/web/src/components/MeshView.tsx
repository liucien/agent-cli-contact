...
function buildEdges(projection: ShellProjection, agents: AgentSnapshot[]): EdgeSpec[] {
  ...
  for (const [k, msgs] of groups) {
    ...
    if (normal.length > 0) {
      ...
      const label =
        injected.length > 0
          ? `prompt ×${normal.length} · 最近 ${fmtTime(latest)}`
          : `prompt ×${normal.length} · 队列中`;
      edges.push({ key: `m-${k}`, from, to, style: 'solid', label, lane: nextLane(from, to) });
    }
    if (held.length > 0) {
      edges.push({
        key: `h-${k}`,
...
function Graph({
  ...
  const edges = useMemo(() => buildEdges(projection, agents), [projection, agents]);
  ...
  return (
    <div className="graph" ref={ref}>
      {agents.length === 0 && <div className="graph-empty">暂无 agent</div>}
      <svg>
        <defs>
          <marker id="arrow-b" markerWidth="9" markerHeight="9" refX="7" refY="3.5" orient="auto">
            <path d="M0,0 L7,3.5 L0,7 z" fill="#7aa2f7" />
          </marker>
          <marker id="arrow-y" markerWidth="9" markerHeight="9" refX="7" refY="3.5" orient="auto">
        ...
        {edges.map((e) => {
          ...
          const dash = e.style === 'dashed' ? '5 4' : e.style === 'wait' ? '3 5' : undefined;
          return (
            <g key={e.key}>
              <path
                d={d}
      ...
      {agents.map((a) => {
        ...
        return (
          ...
          >
            ...
            <div className="meta">
              <span className={`pill ${STATUS_PILL[a.status] ?? 'i'}`}>{a.status}</span>
              {chips.map((c, i) => (
  ...
  if (!pair) {
    return (
      <div className="detail">
        <div className="thread-empty">点击节点或连线查看两个 agent 之间的会话</div>
      </div>
    );
  }
  const [a, b] = pair;
  ...
  return (
    <div className="detail">
      <div className="hd">
        ...
        <div className="sub">
          {msgs.length} 条消息 · 中继审批{projection.mesh.relayApproval ? '已开启' : '已关闭'} ·
          通过 gateway MCP 转发
        </div>
      </div>
      <div className="thread">
        {msgs.length === 0 && <div className="thread-empty">两者之间暂无消息</div>}
        {msgs.map((m) =>
          m.status === 'held' ? (
            <div key={m.id} className="hold">
              <div className="hh">
                待放行 · {partyName(projection, m.from)} → {partyName(projection, m.to)}
              </div>
            ...
            <div key={m.id} className={`bub${m.from === a ? ' rt' : ''}`}>
              ...
              <span className="t">
                {fmtTime(m.createdAt)}
                {m.status === 'injected' ? ' · 已注入' : m.status === 'queued' ? ' · 队列中' : m.status === 'denied' ? ' · 已驳回' : ''}
              </span>
            </div>
          ),
        )}
...
function kindTag(entry: MeshTimelineEntry): { cls: string; label: string } {
  if (entry.kind === 'wait') return { cls: 'w', label: '等待' };
  if (entry.kind === 'message') return { cls: 'p', label: 'prompt' };
  return { cls: 's', label: '状态' };
}
  ...
  return (
    <div className="timeline">
      ...
      <div className="tl-rows">
        {entries.length === 0 && <div className="tl-empty">暂无记录</div>}
        {entries.map((e) => {
          const tag = kindTag(e);
          ...
          if (e.pendingAction === 'approve' && e.messageId) {
            ...
            action = (
              <button
                className="stt hold"
                onClick={() => agentId && selectAgent(agentId, { toWorkbench: true })}
              >
                待处理
              </button>
            );
          } else if (e.kind === 'wait') {
            action = <span className="stt dim">监听中</span>;
          } else if (msg) {
            action =
              msg.status === 'injected' ? (
                <span className="stt ok">已注入</span>
              ) : msg.status === 'queued' ? (
                <span className="stt dim">队列中</span>
              ) : msg.status === 'denied' ? (
                <span className="stt dim">已驳回</span>
              ) : (
                <span className="stt hold">待放行</span>
              );
          }
          return (
            <div key={e.id} className="row">
              ...
              <span className="dir">
                {e.kind === 'status' || e.kind === 'system' ? (
                  <>
                    <b>{partyName(projection, e.from ?? '')}</b> 系统
                  </>
                ) : e.kind === 'wait' ? (
                  <>
              ...
              <span className={`kind ${tag.cls}`}>{tag.label}</span>
...
export function MeshView({
  ...
  const topFilters: { id: TopFilter; label: string }[] = [
    { id: 'all', label: '全部' },
    { id: 'blocked', label: '仅 blocked' },
    { id: 'hour', label: '最近 1 小时' },
    { id: 'sched', label: '定时任务产生' },
  ];
      ...
      <div className="toolbar">
        ...
        <div className="r">
          ...
          <button
            ...
            title="切换 mesh 中继审批"
            onClick={() =>
              void call('mesh.setRelayApproval', { enabled: !projection.mesh.relayApproval })
            }
          />
